...
//                        R.color.colorPrimary);
    ...
    public boolean addLabel(String textLabel, int position) {
        if (!checkLabelsCompleted()) {
            Label label = new Label(getContext(), mTextSize, mIconCross, mShowCross,
                    mTextColor, mBackgroundResource, mLabelsClickables, mLabelPadding, position);
     ...
     * Method to add a Label if is possible.
     * <p>
     * textLabel is the text of the label added.
     */
...
//    }
     ...
     */
    @Override
    public void onClickCross(Label label, int position) {
        removeView(label);
        decreaseLabelsCounter();

        if (listenerOnRemoveLabel != null) {
            if (label.getTag() instanceof Integer) {
                listenerOnRemoveLabel.onRemoveLabel(label, position);
            } else {
                listenerOnRemoveLabel.onRemoveLabel(label, position);
            }
        }
     ...
     */
    @Override
    public void onClickLabel(Label label, int position) {
        if (listenerOnLabelClick != null) {
            listenerOnLabelClick.onClickLabel(label, position);
        }
    }
            ...
            if (labelsAdded != null) {
                for (int i = 0; i < labelsAdded.size(); i++) {
                    ...
                    if (labelValues.getKey() == -1) {
                        addLabel(labelValues.getValue(), i);
                    } else {
    ...
    public interface OnLabelClickListener {
         ...
         */
        void onClickLabel(Label labelClicked, int position);
    }
}