...
//import com.pronovoscm.camerakit.DisplayOrientationDetector;
...
//    private DisplayOrientationDetector mDisplayOrientationDetector;
    ...
    private void onGesture(GestureLayout source, @NonNull CameraOptions options) {
        ...
        switch (action) {
            ...
            case CAPTURE:
                mCameraController.capturePicture(null);
                break;
    ...
    protected boolean checkPermissions(SessionType sessionType, Audio audio) {
        ...
        needsCamera = needsCamera && BaseActivity.checkSelfPermission(c, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED;
        needsAudio = needsAudio && BaseActivity.checkSelfPermission(c, Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED;
     ...
     * - {@link #capturePicture(CameraControls.OnImageCapturedCallback)} might fallback to {@link #captureSnapshot()} or might not work
     *
     ...
     * @see #captureSnapshot()
     */
    public void capturePicture(CameraControls.OnImageCapturedCallback cameraControls) {
        mCameraController.capturePicture(cameraControls);
    }
     ...
     * was registered.
     * <p>
     * The difference with {@link #capturePicture(CameraControls.OnImageCapturedCallback)} is that this capture is faster, so it might be
     * better on slower cameras, though the result can be generally blurry or low quality.
     *
     * @see #capturePicture(CameraControls.OnImageCapturedCallback)
     */
    ...
    interface CameraCallbacks extends OrientationHelper.Callback {
        ...
        void processImage(byte[] jpeg, boolean consistentWithView, boolean flipHorizontally, CameraControls.OnImageCapturedCallback cameraControls);
    ...
    private class Callbacks implements CameraCallbacks {
        ...
        public void onShutter(boolean shouldPlaySound) {
            ...
            mShutterView.playShutterEffect();
        }
         ...
         */
        @Override
        public void processImage(final byte[] jpeg, final boolean consistentWithView, final boolean flipHorizontally, CameraControls.OnImageCapturedCallback cameraControls) {
            ...
            mWorkerHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    cameraControls.onImageCapture(jpeg2);
//                    mCameraController.getCameraOptions();
        ...
        public void onDeviceOrientationChanged(int deviceOrientation) {
            ...
            mUiHandler.post(new Runnable() {
                @Override
                public void run() {
                    // TODO: 16/10/18 sanjay orientation
                    if (context instanceof PronovosCameraActivity) {
                        ((PronovosCameraActivity) (context)).onDeviceOrientationChanged(deviceOrientation);
                    }
                    for (CameraListener listener : mListeners) {
