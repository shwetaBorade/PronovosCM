...
public class CameraUtils {
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public static void decodeBitmap(final byte[] source, boolean isLandscape, Context context, final BitmapCallback callback) {
        if (isLandscape(context)) {
            decodeBitmap(source, Integer.MAX_VALUE, Integer.MAX_VALUE, callback);
        } else {
            callback.onBitmapReady(BitmapFactory.decodeByteArray(source, 0, source.length));
        }
    }
