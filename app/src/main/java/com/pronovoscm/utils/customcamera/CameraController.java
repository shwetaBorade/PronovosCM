...
abstract class CameraController implements
    ...
    abstract void capturePicture(CameraControls.OnImageCapturedCallback cameraControls);
    ...
    protected final int calculateCaptureRotation() {
        int captureRotation = 0;
        if (mFacing.value() == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            captureRotation = (mSensorOffset + mDisplayOffset) % 360;
        } else {  // back-facing camera
            captureRotation = (mSensorOffset - mDisplayOffset + 360) % 360;
        }
        ...
        if (mFacing.value() == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            captureRotation = ((captureRotation - (mDisplayOffset - mDeviceOrientation)) + 360) % 360;
        } else {  // back-facing camera
            captureRotation = (captureRotation + (mDisplayOffset - mDeviceOrientation) + 360) % 360;
        }
//        captureRotation = captureRotation + 90;
