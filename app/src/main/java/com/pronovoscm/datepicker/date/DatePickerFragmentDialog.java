...
import com.pronovoscm.datepicker.Utils;
...
public class DatePickerFragmentDialog extends DialogFragment implements
        OnClickListener, DatePickerController, OnChangeDate {
    ...
    private static SimpleDateFormat MONTH_FORMAT_NEW = new SimpleDateFormat("MMM", Locale.getDefault());
    ...
    public static DatePickerFragmentDialog newInstance(OnDateSetListener callback,Calendar calendar) {
//        Calendar now = Calendar.getInstance();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final Activity activity = getActivity();
        int widthInPixel = (int) (activity.getResources().getDisplayMetrics().widthPixels * 0.8);
        int heightPixels = (int) (activity.getResources().getDisplayMetrics().heightPixels * 1 / 8);
        ...
        mDayPickerView = new SimpleDayPickerView(activity, this, this);

        yearPickerView = new YearPickerView(activity, this);

        mYearPickerPopup = new ListPopupWindow(mYearView.getContext());
        mYearPickerPopup.setAnchorView(mYearView);

        mYearPickerPopup.setHeight((activity.getResources().getDisplayMetrics().heightPixels*1/2));
        mYearPickerPopup.setContentWidth(250);
        ...
        int bgColorResource = mThemeDark ? R.color.amdp_date_picker_view_animator_dark_theme
                : R.color.amdp_date_picker_view_animator;
        ...
        TextView okButton = view.findViewById(R.id.amdp_ok);
        ...
        mAccentColor = getResources().getColor(R.color.color_accent);
        ...
        LinearLayout linearLayout = view.findViewById(R.id.parentView);
        LinearLayout linearLayout1 = view.findViewById(R.id.amdp_day_picker_selected_date_layout);



        mAnimator.setLayoutParams(new FrameLayout.LayoutParams(widthInPixel, LinearLayout.LayoutParams.WRAP_CONTENT));
        linearLayout1.setLayoutParams(new LinearLayout.LayoutParams(widthInPixel, heightPixels *3/4));
        linearLayout.setBackgroundResource(R.drawable.rounded_corner_bg);
        linearLayout.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT));
        return view;
    }
    ...
    public void onConfigurationChanged(final Configuration newConfig) {
        ...
        if (window !=null && window.isShowing()){
            window.dismiss();
        }if (mYearPickerPopup!=null && mYearPickerPopup.isShowing()){
        ...
        if (viewGroup != null) {
            ...
            mMonthPickerView.setText(MONTH_FORMAT_NEW.format(mCalendar.getTime()));
    ...
    public void setTimeZone(TimeZone timeZone) {
        ...
        MONTH_FORMAT_STAND_ALONE.setTimeZone(timeZone);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.amdp_date_picker_year) {
            ...
            for (int i = 0; i < 12; i++) {
                ...
                monthList.add(MONTH_FORMAT_STAND_ALONE.format(calendar.getTime()));
            }

            window = new ListPopupWindow(getActivity());
            ArrayAdapter<String> adapter = new ArrayAdapter<String>(getActivity(),
                    R.layout.amdp_year_label_text_view, monthList);
            /* use ur custom layout which has only TextView along with style required*/
            window.setAdapter(adapter);
            window.setHeight((getActivity().getResources().getDisplayMetrics().heightPixels*1/2));
             window.setAnchorView(mMonthPickerView);

            window.setContentWidth(300);/* set width based on ur requirement*/
...
//                    mMonthPickerView.setText(monthList.get(i));
                    mMonthPickerView.setText(MONTH_FORMAT_NEW.format(mCalendar.getTime()));
                    window.dismiss();
                }
            });
    ...
    @Override
    public void getChangeYear(String year) {
        setYearText(year);
    }
}

interface OnChangeDate {
    void getChangeDate(String date);

    void getChangeYear(String year);
}
