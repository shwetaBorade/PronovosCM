...
public class FontResource {
    ...
    public static Single<Set<String>> writeFontAssetsToCache(@NonNull Context context, @NonNull Set<String> fonts) {
        return Single.create(new SingleOnSubscribe<Set<String>>() {
            @Override
            public void subscribe(@NonNull SingleEmitter<Set<String>> emitter) throws Exception {
                HashSet<String> result = new HashSet<>(fonts.size());
                for (String font : fonts) {
                    File outputFile = Utils.copyAssetsToTempFolder(context, font, FONT_FOLDER, true);
                    if (outputFile != null) {
    ...
    public static Single<Set<String>> writeFontFileToCache(@NonNull Context context, @NonNull Set<String> fonts) {
        File fontCacheFolder = new File(context.getCacheDir(), FONT_FOLDER);
        return Single.create(new SingleOnSubscribe<Set<String>>() {
            @Override
            public void subscribe(@NonNull SingleEmitter<Set<String>> emitter) throws Exception {
                FileUtils.forceMkdir(fontCacheFolder);
                HashSet<String> result = new HashSet<>(fonts.size());
                for (String font : fonts) {
                    ...
                    try {
                        ...
                        String fileName = FilenameUtils.getName(font);
                        File outputFile = new File(fontCacheFolder, fileName);
