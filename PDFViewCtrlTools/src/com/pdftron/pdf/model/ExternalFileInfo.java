...
public class ExternalFileInfo implements Cloneable, BaseFileInfo {
    ...
    public static String getParentRelativePath(Uri uri, String name) {
        String parentRelativePath = Utils.getUriDocumentPath(uri);
        if (!Utils.isNullOrEmpty(parentRelativePath)) {
            // Remove the file/folder's name from the end of the path
            if (parentRelativePath.endsWith(name)) {
                parentRelativePath = parentRelativePath.substring(0, parentRelativePath.length() - name.length());
                // Remove trailing "/"
