...
public class LoadFontAsyncTask extends CustomAsyncTask<Void, Void, ArrayList<FontResource>> {
    private final ArrayList<FontResource> mFonts = new ArrayList<>();
    private Set<String> mWhiteListFonts;
    private Callback mCallback;
    private boolean mIsCustomFonts;
    ...
    private void loadFontResources() {
        ...
        if (mFonts.isEmpty() && mIsCustomFonts && mWhiteListFonts != null && !mWhiteListFonts.isEmpty()) {
            // no fonts were loaded, however custom fonts were passed in
            for (String f : mWhiteListFonts) {
                String fileName = FilenameUtils.getName(f);
                FontResource fontResource = new FontResource(fileName, f, fileName, fileName);
                mFonts.add(fontResource);
            }
        }
