...
public class PopulateUserBookmarkListTask extends CustomAsyncTask<Void, Void, Void> {
     ...
     */
    public PopulateUserBookmarkListTask(@NonNull Context context, String filePath, Bookmark bookmark, boolean readOnly, String queryText) {
        super(context);
        mBookmark = bookmark;
        mFilePath = filePath;
        mReadOnly = readOnly;
    ...
    protected Void doInBackground(Void... params) {
        if (mReadOnly) {
            mBookmarkList.addAll(BookmarkManager.getUserBookmarks(getContext(), mFilePath, mQueryText));
            if (mBookmarkList.isEmpty()) {
                mBookmarkList.addAll(BookmarkManager.getPdfBookmarks(mBookmark, mQueryText));
            }
        } else {
            mBookmarkList.addAll(BookmarkManager.getPdfBookmarks(mBookmark, mQueryText));
            if (mBookmarkList.isEmpty()) {
