...
public class FindTextTask extends AsyncTask<Void, Boolean, Integer> {
            ...
            try {
                ...
                while (!isCancelled()) {
                    ...
                    if (result.getCode() == TextSearchResult.e_found) {
                        ...
                        ArrayList<Double> resultQuads = new ArrayList<>();
                        while (highlights.hasNext()) {
                            ...
                            Double[] doubleArray = ArrayUtils.toObject(quads);
                            List<Double> list = Arrays.asList(doubleArray);
                            resultQuads.addAll(list);
                            highlights.next();
                        }
                        mHighlights.put(result, resultQuads);
    ...
    protected void onPostExecute(Integer numResults) {
        if (mCallback != null) {
            mCallback.onFindTextTaskFinished(numResults, mResults, mHighlights);
        }
    }
