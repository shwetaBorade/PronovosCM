...
public class ThumbnailSlider extends LinearLayout implements
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        View barLayout = findViewById(R.id.controls_thumbnail_slider_scrubberview);
        mShadowView = findViewById(R.id.controls_thumbnail_shadow_view);
        ...
        mRootView = (ConstraintLayout) LayoutInflater.from(context).inflate(R.layout.controls_thumbnail_slider_preview, null);
        mThumbnailViewImage = mRootView.findViewById(R.id.controls_thumbnail_slider_thumbview_thumb);
        mPageIndicator = mRootView.findViewById(R.id.controls_thumbnail_slider_thumbview_pagenumber);

        mThumbnailViewPopup = new PopupWindow(
                mRootView,
                (int) (mThumbViewHeight / SCREEN_RATIO_THUMB_SIZE),
                (int) (mThumbViewHeight / SCREEN_RATIO_THUMB_SIZE)
        );
        mThumbnailViewPopup.setAnimationStyle(android.R.style.Animation_Dialog);
        ...
        mSeekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
            ...
            public void onStartTrackingTouch(SeekBar seekBar) {
                if (mPdfViewCtrl != null) {
                    ...
                    int posY = sliderPosition[1] - (int) Utils.convDp2Pix(getContext(), 2) - mThumbnailViewPopup.getHeight();
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                if (mPdfViewCtrl != null) {
                    String currentPageLabel = PageLabelUtils.getPageLabelTitle(mPdfViewCtrl, currentPage);
        ...
        try {
            ...
            // background color
            int backgroundColor = typedArray.getColor(R.styleable.ThumbnailSlider_colorBackground, 0);
            if (backgroundColor != 0) {
                barLayout.setBackgroundColor(backgroundColor);
            }

            // elevation
            boolean showShadow = typedArray.getBoolean(R.styleable.ThumbnailSlider_shadowEnabled, true);
    ...
    private void setThumbnailViewImageBitmap(Bitmap bitmap, int resId, int width, int height) {
        ...
        try {
            if (mThumbnailViewImage != null) {
                ...
                // Here we update the constraints so that we can fit the bitmap into the image view
                // without padding, in order to support padding around the image
                ...
                if (width > height) {
                    ...
                    constraintSet.constrainHeight(R.id.controls_thumbnail_slider_thumbview_thumb_container, ConstraintSet.WRAP_CONTENT);
                    constraintSet.clear(R.id.controls_thumbnail_slider_thumbview_thumb_container, ConstraintSet.TOP);
                } else {
                    constraintSet.constrainWidth(R.id.controls_thumbnail_slider_thumbview_thumb_container, ConstraintSet.WRAP_CONTENT);
                    constraintSet.constrainHeight(R.id.controls_thumbnail_slider_thumbview_thumb_container, 0);
                    constraintSet.connect(R.id.controls_thumbnail_slider_thumbview_thumb_container, ConstraintSet.TOP, ConstraintSet.PARENT_ID, ConstraintSet.TOP);
                }
