...
public class BookmarksTabLayout extends CustomFragmentTabLayout implements
    ...
    private int mTabLayoutBackgroundDialog;
    private int mTabLayoutBackgroundSheet;
    ...
    public BookmarksTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.custom_bookmarks_tab_layout);
    }
    ...
    public BookmarksTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        init(context, attrs, defStyleAttr, R.style.BookmarksTabLayoutStyle);
        mShouldMemorizeTab = false;
    }

    private void init(@NonNull Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        // initialize colors
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.BookmarksTabLayout, defStyleAttr, defStyleRes);
        try {
            mTabTintColorDialog = typedArray.getColor(R.styleable.BookmarksTabLayout_tabTintColorDialog, Color.WHITE);
            mTabTintColorSheet = typedArray.getColor(R.styleable.BookmarksTabLayout_tabTintColorSheet, context.getResources().getColor(R.color.navigation_list_icon_color));
            mTabTintSelectedColorDialog = typedArray.getColor(R.styleable.BookmarksTabLayout_tabTintSelectedColorDialog, Color.WHITE);
            mTabTintSelectedColorSheet = typedArray.getColor(R.styleable.BookmarksTabLayout_tabTintSelectedColorSheet, Utils.getAccentColor(context));
            mTabLayoutBackgroundDialog = typedArray.getColor(R.styleable.BookmarksTabLayout_tabBackgroundDialog, Utils.getPrimaryColor(context));
            mTabLayoutBackgroundSheet = typedArray.getColor(R.styleable.BookmarksTabLayout_tabBackgroundSheet, Utils.getBackgroundColor(context));
            if (mTabTintColorDialog == mTabTintSelectedColorDialog) {
                mTabTintColorDialog = Utils.adjustAlphaColor(mTabTintSelectedColorDialog, .5f);
            }
        } finally {
            typedArray.recycle();
        }
    ...
    /**
     * @hide The overloaded implementation of {@link CustomFragmentTabLayout#startFragment(String)}.
     **/
    @Override
    ...
    public void onPrepareMenu(Menu menu, Fragment fragment) {
        if (fragment instanceof AnnotationDialogFragment) {
            AnnotationDialogFragment annotationFrag = (AnnotationDialogFragment) fragment;
            annotationFrag.prepareOptionsMenu(menu);
        } else if (fragment instanceof UserBookmarkDialogFragment) {
            UserBookmarkDialogFragment userBookmarkFrag = (UserBookmarkDialogFragment) fragment;
            userBookmarkFrag.prepareOptionsMenu(menu);
        } else if (fragment instanceof OutlineDialogFragment) {
            OutlineDialogFragment outlineDialogFragment = (OutlineDialogFragment) fragment;
            outlineDialogFragment.prepareOutlineSearch(menu);
        }
    }
    ...
    public boolean onMenuItemClicked(MenuItem item, Fragment fragment) {
        if (fragment instanceof AnnotationDialogFragment) {
            AnnotationDialogFragment annotationFrag = (AnnotationDialogFragment) fragment;
            return annotationFrag.onOptionsItemSelected(item);
        } else if (fragment instanceof OutlineDialogFragment) {
    ...
    public void onEditBookmarkFocusChanged(boolean isActive) {
        mBookmarksTabsListener.onEditBookmarkFocusChanged(isActive);
    }
