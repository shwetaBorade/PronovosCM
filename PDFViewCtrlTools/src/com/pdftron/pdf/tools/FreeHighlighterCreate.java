...
public class FreeHighlighterCreate extends SimpleShapeCreate {
    ...
    public static final float BLEND_OPACITY = .8f;
    public static final float AUTO_SMOOTH_RANGE_DEFAULT = 20f;
    ...
    // auto smooth
    private double mAutoSmoothingRange;
    ...
    public FreeHighlighterCreate(@NonNull PDFViewCtrl ctrl) {
        ...
        mAutoSmoothingRange = Utils.convDp2Pix(mPdfViewCtrl.getContext(), ((ToolManager) mPdfViewCtrl.getToolManager()).getFreeHighlighterAutoSmoothingRange());
    }
     ...
     *
     * @param range indicating how wide is the range to auto smooth vertical and horizontal strokes, in dp.
     */
    public void setAutoSmoothingRange(float range) {
        mAutoSmoothingRange = Utils.convDp2Pix(mPdfViewCtrl.getContext(), range);
    }

    public void applyAutoSmooth() {
        boolean autoSmoothY = true;
        boolean autoSmoothX = true;
        float sumX = 0;
        float sumY = 0;
        if (!mScreenStrokePoints.isEmpty()) {
            PointF firstPoint = mScreenStrokePoints.get(0);
            for (PointF point : mScreenStrokePoints) {
                sumX += point.x;
                sumY += point.y;
                if (point.y > firstPoint.y + this.mAutoSmoothingRange || point.y < firstPoint.y - this.mAutoSmoothingRange) {
                    autoSmoothY = false;
                }
                if (point.x > firstPoint.x + this.mAutoSmoothingRange || point.x < firstPoint.x - this.mAutoSmoothingRange) {
                    autoSmoothX = false;
                }
            }
            float averageY = sumY / mScreenStrokePoints.size();
            float averageX = sumX / mScreenStrokePoints.size();
