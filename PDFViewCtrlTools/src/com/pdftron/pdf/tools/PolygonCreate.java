...
public class PolygonCreate extends AdvancedShapeCreate {
    ...
    protected void drawMarkup(@NonNull Canvas canvas,
        ...
        if (mBorderStyle == ShapeBorderStyle.CLOUDY) {
            DrawingUtils.drawCloudyPolygon(mPdfViewCtrl, getPageNum(), canvas, canvasPoints,
                    mPath, mPaint, mStrokeColor, mFillPaint, mFillColor, CloudCreate.BORDER_INTENSITY);
        } else {
            DrawingUtils.drawPolygon(mPdfViewCtrl, getPageNum(), canvas,
                    canvasPoints, mPath, mPaint, mStrokeColor,
                    mFillPaint, mFillColor, mBorderStyle == ShapeBorderStyle.DASHED ? mDashPathEffect : null);
        }
    }
}
